...
const _sfc_main = {
  ...
  methods: {
    ...
    async uploadBatchImage(index, tempPath) {
      try {
        const imageUrl = await common_utils_ossUpload.uploadToOss(tempPath);
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    s: common_vendor.t(($data.strength * 100).toFixed(0)),
    t: $data.strength * 100,
    v: common_vendor.o((...args) => $options.onStrengthChange && $options.onStrengthChange(...args)),
    w: common_vendor.t($data.generating ? "批量修复中..." : `批量修复(${$data.imageList.length}张)`),
    x: $data.imageList.length === 0 || $data.generating,
    y: common_vendor.o((...args) => $options.batchRestore && $options.batchRestore(...args))
  }));
}
